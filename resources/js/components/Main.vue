<template>
  <div>
    ...
    <div v-if="mediaPresent" class="card p-4 mt-4 content">
      <h2>Media Usage</h2>
      <p>Media consumed between {{ date_start }} and {{ date_end }}</p>
      <table class="mt-2">
        <thead>
        <tr>
...
export default {
  ...
  props: ['dbStats', 'dbDownloads', 'dbMedia', 'fetchUrlStats', 'fetchUrlDownloads', 'fetchUrlMediaUsage'],
  data() {
    return {
      message: "Test 2?",
      rawStats: this.dbStats,
      fileDownloads: this.dbDownloads,
      mediaUsage: this.dbMedia,
      ui: {
        sessions: true,
        views: true,
        downloads: false,
  ...
  computed: {
    ...
    mediaPresent() {
      return this.mediaUsage.length > 0;
    }
  },
  methods: {
    echoData() {
    ...
    fetchData() {
      ...
      const requestMediaUsage = new Request(`${this.fetchUrlMediaUsage}?start=${this.date_start}&end=${this.date_end}`);
      fetch(requestStats)
      ...
      fetch(requestMediaUsage)
          .then((response) => response.json())
          .then((data) => {
            this.mediaUsage = data;
          })
          .finally(() => {
            this.fetchingMediaUsage = false;
          });
    },
    toggleLine(line) {
    ...
    parseStats() {
      ...
      this.rawStats.forEach(stat => {
        ...
        this.lines.media.data.push(stat.media);
        ...
        this.sums.media += parseInt(stat.media);
